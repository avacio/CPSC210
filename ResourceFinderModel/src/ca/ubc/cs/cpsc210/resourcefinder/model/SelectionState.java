...
public class SelectionState {
    ...
    public Set<Resource> getResourcesWithSelectedServices() {
        Set AllResources = new HashSet(registry.getResources());

        if (selected.size() == 0) {return AllResources;}

        if (isAnySelected) {
            return registry.getResourcesOfferingAnyServicesInSet(selected);}
            else  { return registry.getResourcesOfferingAllServicesInSet(selected);
        }
    }
}
